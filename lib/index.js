...
function fetchApartments({
  hostOpts = {
    ...
    query: {
      search_distance: 0,
      postal: process.env.POSTAL_CODE || 20010,
      availabilityMode: 0,
      map: 1
    }
  },
  notBeforeUnixDate = null,
  latLngFilter = null
} = {}) {
  ...
  return r.getAsync({
    ...
    .then(resp => {
      ...
      if (notBeforeUnixDate) {
        matches = _.filter(matches, match => match.PostedDate > notBeforeUnixDate);
      }
...
function formatSlackMessage({parsed: {text, images, time, mapAddresses, price}, link}, {maxImages=2} = {}) {
  let template = {
    fallback: 'new apt!'
  };
